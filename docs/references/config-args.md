...
| <a name="IBGA_SCREEN_SIZE">`IBGA_SCREEN_SIZE`</a> | Xvfb virtual-screen geometry in the form `WIDTHxHEIGHTxDEPTH` (e.g. `1600x1200x16`). Defaults to `1280x1024x16`. The default is sized to fit IB Gateway's tallest configuration dialogs (including Configuration/API/Settings) so the option-check automation can reach the Apply/OK buttons. Lower values may cause bottom buttons to render off-screen, silently breaking settings changes. |
